...
public class JavaEditionFragment extends BaseFragment {
    
    private List<NavigationTab> navigationTabs;
    
    public JavaEditionFragment() {
        super("Java Edition");
    }
    ...
    private StackPane createGameContent() {
        ...
        try {
            ...
            titleView.setTranslateY(-160); 
            titleView.setFitWidth(300);
            
            titleContainer.getChildren().add(titleView);
        } catch (Exception e) {
            Label fallbackTitle = new Label("MINECRAFT");
            ...
            titleContainer.getChildren().addAll(fallbackTitle, subtitle);
        }
